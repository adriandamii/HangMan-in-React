        ...
        <div id="lives">
            {hangArrImages.map((hangNr, index) => {
                if (index === props.lives && props.win === false) {
                    return <img key={index} src={hangNr} className= "hangman"/>
                } 
            })}
            {props.win ? <img src={winHM} className="hangman"/> : null}
            {props.win ? <h3 id="win">Congratulations!</h3> : null}
            {props.lives === 0 ? <h3 id="gameOverWord">The word is {props.inputValue}</h3> : null}
        </div>
    )
}